...
The goal is to combine multiple ECCC hourly datasets in their 'wide' format. In other words, so that each row is a day, and the hourly data is sorted as a column.  
...
combineO3andNO2 <- function(O3, NO2, nrows = Inf){
  ...
  O3dat <-  read_csv(O3, skip = 7, n_max = nrows) %>%
    ECCCcolRename(., "O3") %>%
    select(-c("Pollutant"))
    
  NO2dat <- read_csv(NO2, skip = 7, n_max = nrows) %>%
    ECCCcolRename(., "NO2") %>%
