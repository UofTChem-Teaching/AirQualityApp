...
I wrote another function which will combine tidied *O~3~* and *NO~2~* datasets,^[Note this uses inner_join, so only O3 and NO2 values from stations found in BOTH datasets will be included.] and calculate the *O~x~* value at each given time. All `-999` values are converted to `NA`, and therefore are not used in any subsequent plotting/calculations. I also added a `rows` input where you can specify the number of rows you want to import from the `.csv`. The default is `Inf` (read every row), but you can specify smaller numbers when testing stuff. 
