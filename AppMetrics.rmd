...
The CHM135 [Air Quality App](https://davidrosshall.shinyapps.io/AirQualityApp/) has been used for during the Winter 2021 and Fall 2021 iterations of the virtual CHM135 labs. Combing through the sparse Shiny App metrics for an understanding of app use, etc.
...
The Shiny server records the number of "connections", which can be interpreted as "an indivdual person connects to the app website". Note that there can be multiple connections for a given individual (i.e. they go to the website on two different days). Nonetheless, it provides a useful approximate for the number of people who've used the app. Figure 1 shows the app connections over time and days for both the Winter and Fall 2021 iterations. Predictably, the most connection are towards the tail end of the *Practical 1* synchronous sessions (Jan 18th to 22rd), and peaking over the weekend. 

```{r app-connections_data, eval = FALSE, echo = FALSE, warning = FALSE, error = FALSE, message=FALSE}
...
```{r app-connections_functions, echo = FALSE, error = FALSE, message = FALSE, warning = FALSE}
...
cumsum.plot(Fall2021)

daycount.plot(Fall2021)
...
fall1 <- fall %>%
  select(-timestamp) %>% 
  filter(date > ymd_hms("2021-09-17 00:00:00") & date < ymd_hms("2021-10-18 00:00:00")) %>%
  arrange(date) %>% 
  mutate(
    n_count=cumsum(connect_count),
    n_procs=cumsum(connect_procs),
